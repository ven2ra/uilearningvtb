...
const { coin, crystalRobot, bulb } = lessonAssets;
...
export function LessonIllustration({ visual }: { visual: LessonVisual }) {
  if (visual === 'inflation') return <><div className="lf-baskets"><div><b>Сегодня</b><img src={lessonAssets.basketToday} alt="Полная корзина продуктов"/><strong>100 000 ₽</strong></div><span aria-hidden="true">→</span><div><b>Через несколько лет</b><img src={lessonAssets.basketFuture} alt="Меньше продуктов в корзине"/><strong>100 000 ₽</strong></div></div><LessonInfoCard>Это влияние инфляции: со временем на одну и ту же сумму можно приобрести меньше товаров и услуг.</LessonInfoCard></>;
  if (visual === 'instruments') return <div className="lf-instruments">{(['coins', 'chart', 'pie'] as const).map((name, i) => <div key={name}><div><img src={lessonAssets[name]} alt=""/></div><b>{['Облигации', 'Акции', 'Фонды'][i]}</b></div>)}</div>;
  return <div className={`lf-scene lf-scene-${visual}`}><img className="lf-decor" src={lessonAssets.decor} alt=""/><img className="lf-robot" fetchPriority="high" decoding="async" src={visual === 'crystal' ? crystalRobot : lessonAssets[visual === 'risk' ? 'warning' : 'welcome']} alt="Маскот ФинКод"/><img className="lf-spark one" src={coin} alt=""/><img className="lf-spark two" src={coin} alt=""/></div>;
}
