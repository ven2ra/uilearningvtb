...
export default function LearningPath() {
  ...
  const openLesson = (i: number) => { if (i <= completed) { setIncorrect(false); setSelected(i); } };
  const setStep = (next: number) => { if (selected === null) return; setIncorrect(false); app.setCourse(c => ({ ...c, steps: { ...c.steps, [selected]: next } })); };
  const submit = () => {
    if (selected === null || !lesson || answer !== lesson.correct) { setIncorrect(true); return; }
    app.setCourse(c => {
      if (selected > c.completed || c.answers[selected] !== COURSE[selected].correct) return c;
      const first = selected === c.completed;
      const today = new Date().toLocaleDateString("en-CA");
      return { ...c, completed: first ? c.completed + 1 : c.completed, coins: c.coins + (first ? 50 : 0), days: first && !c.days.includes(today) ? [...c.days, today] : c.days, steps: { ...c.steps, [selected]: 3 } };
    });
    setIncorrect(false);
  };
  let streak = 0;
  ...
  return <div className="learning-path">
    ...
    <div className="learning-path-stats">
      ...
      <section><strong><StatIcon index={1} /> {app.course.coins}</strong><small>финкоинов</small></section>
      <button onClick={() => app.go("achievements")}><strong><StatIcon index={2} /> {app.achievements.length} <Icon name="chevronRight" size={14} /></strong><small>достижения</small></button>
    </div>
    ...
    <DetailSheet detail={selected === null || !lesson ? null : { title: lessons[selected].title, body: <div className="course-lesson">
      ...
      </> : <><p>Тест пройден. {lesson.explanation}</p><p>Награда за этот урок: 50 финкоинов. При повторном прохождении она не начисляется.</p><div className="course-controls"><button onClick={() => { app.setCourse(c => ({ ...c, steps: { ...c.steps, [selected]: 0 }, answers: { ...c.answers, [selected]: -1 } })); }}>Повторить урок</button><button className="course-primary" onClick={() => setSelected(null)}>К урокам</button></div></>}
