...
export function SourceProfile() {
  ...
  return <div data-tour="profile-tour" className="snapshot-profile"><Header title="Профиль" /><div className="snapshot-profile-body">
    ...
    {tab === "Профиль" ? <><section className="snapshot-profile-card"><h2>Данные аккаунта</h2><dl className="snapshot-personal">{[["ФИО", "Долгасова А.Г"], ["Телефон", "+7999*****57"], ["Электронная почта", "kirillkicked1@gmail.com"], ["Статус квал. инвестора", "Присвоен"]].map(([label, value], i) => <div key={label}><dt>{label}</dt><dd>{value}{i === 3 && <Glyph id={8} />}</dd></div>)}</dl><div className="snapshot-logout">Выйти из аккаунта<Glyph id={9} /></div></section><Agreement /><Agreement iis /></> : <section className="snapshot-profile-card snapshot-profile-list">
      {(tab === "Настройки" ? ["Безопасность", "Изменить пароль", "Скрывать баланс при входе", "О приложении", "Очередь заявок", "Нажатие на цену", "Оформление", "Тема интерфейса", "Мигание котировок", "Новый стиль интерфейса"] : tab === "Услуги" ? ["ИИС", "Новый субсчет", "Маржинальная торговля", "Инвестиционное консультирование", "Тестирование инвесторов", "Перевод ценных бумаг", "Голосования", "Счета для выплат по ценным бумагам", "Процент на остаток", "Терминалы Quik"] : ["Отчеты и справки", "Реквизиты для пополнения", "Смена типа ТКС"]).map(label => label === "Новый стиль интерфейса" ? <div className="snapshot-theme-setting" key={label}><span id="profile-theme-label">{label}</span><button type="button" className="snapshot-theme-switch" role="switch" aria-labelledby="profile-theme-label" aria-checked={app.activeTheme === "crystal"} onClick={() => app.setActiveTheme(app.activeTheme === "crystal" ? null : "crystal")}><span /></button></div> : <div key={label}>{label}</div>)}
    </section>}
  </div></div>;
}
